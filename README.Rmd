...
**This example was particularly challenging to implement because of the lack of documentation, fragmented data files,
and inconsistent state naming conventions. The details on how to create the datasets for this example are in
the script `region-codes/organize-codes.R`.**

```{r avg_expenditure, fig.width=7, fig.height=7}
library(dplyr)
library(duckdb)
library(sf)
library(ggplot2)
